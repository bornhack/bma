...
def bma_startup(sender: "WSGIHandler", **kwargs: dict[str, str]) -> None:
    ...
    _creator_group, created = Group.objects.get_or_create(name=settings.BMA_CREATOR_GROUP_NAME)
    if created:
        logger.info(f"Created creator group {settings.BMA_CREATOR_GROUP_NAME}")

    # create moderator group if needed
    _moderator_group, created = Group.objects.get_or_create(name=settings.BMA_MODERATOR_GROUP_NAME)
    if created:
        logger.info(f"Created moderator group {settings.BMA_MODERATOR_GROUP_NAME}")

    # create creator group if needed
    _curator_group, created = Group.objects.get_or_create(name=settings.BMA_CURATOR_GROUP_NAME)
    if created:
        logger.info(f"Created curator group {settings.BMA_CURATOR_GROUP_NAME}")

    # create worker group if needed
    _worker_group, created = Group.objects.get_or_create(name=settings.BMA_WORKER_GROUP_NAME)
    if created:
