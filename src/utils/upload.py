...
def get_upload_path(instance: "BaseFile", filename: str) -> Path:
    """Return the upload path under MEDIA_ROOT for original files.

    Used by Image, Video, Audio and Document models filefields.
    ...
    return Path(
        ...
        f"user_{instance.uploader.shortuuid}",
        ...
        f"bma_{instance.filetype}_{instance.shortuuid}.{extension}",
    )
...
def get_thumbnail_source_path(instance: "ThumbnailSource", filename: str) -> Path:
    ...
    return Path(
        ...
        f"user_{instance.basefile.uploader.shortuuid}",
        # and under a filetype-specific dir (image, video, audio, or document)
        instance.basefile.filetype,
        # under a file-specific dir
        f"bma_{instance.basefile.filetype}_{instance.basefile.shortuuid}",
        # in a file named thumbnail with the proper extension
        f"thumbnailsource_{instance.shortuuid}.{extension}",
    )
...
def get_thumbnail_path(instance: "Thumbnail", filename: str = "") -> Path:
    ...
    return Path(
        ...
        "thumbnails",
        # inside an AR folder
        f"{instance.aspect_ratio.replace('/', '_')}",
        # with a filename like 400w.webp
        f"thumbnail_{instance.width}w_{instance.shortuuid}.{extension}",
    )
...
def get_image_version_path(instance: "ImageVersion", filename: str) -> Path:
    ...
    return Path(
        ...
        "image",
        # under a file-specific dir
        f"bma_image_{instance.image.shortuuid}",
        # inside an AR folder
        f"{instance.aspect_ratio.replace('/', '_')}",
        # with a filename like 400w.webp
        f"imageversion_{instance.width}w_{instance.shortuuid}.{extension}",
    )
